...
\vspace{0.35em}
\noindent\textbf{Live bus trackers expose route, vehicle, stop, and prediction data, but a rider may still need to translate those data into a personal action. We built a location-first WRTA interface that binds saved Worcester destinations to nearby stops, live buses, route geometry, current location, and walking estimates on one stable map. The contribution is not a new transit authority data source. It is a source-bounded interaction layer that separates official stop-time predictions from derived destination-arrival estimates, reduces map interpretation work, and makes uncertainty visible for a rider who wants fewer choices and less context switching.}
...
The AO Labs bus tracker was built as a narrower interface for that translation. The public version at `bus.aolabs.io' tracks WRTA routes 2, 3, 4, and 31 and six fixed Worcester-area destinations: 96 William Street, Alden Hall, Union Station, Chipotle on Park Avenue, Cold Stone on Main Street, and Blackstone Theaters. It uses WRTA live and stop-time surfaces, Ride Guide route and stop geometry, OpenStreetMap tiles, browser geolocation, Leaflet rendering, and OSRM walking estimates \cite{rideGuide,gtfsSchedule,gtfsRealtimeBestPractices,osm,leaflet,geolocationApi,osrm}. The app is therefore not authoritative beyond the sources it reads. It is a personal layer that narrows those sources into one map and a short set of labeled times.
...
The WRTA tracker is route-first: routes and vehicles are the primary objects. The AO Labs interface is location-first: a selected place reframes the map around the rider's current location, the selected destination, the boarding stop chosen from that current location, the exit stop near the destination, route geometry, and live buses when browser GPS is available (Fig.~\ref{fig:bus_interface}). This changes the task from inspecting the transit network to answering a personal question: which stop, route, and bus time matter for this trip?

\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{bus-interface.png}
    \caption{Stable live interface for selected WRTA destinations. The public app keeps saved destinations, route geometry, bus-stop symbols, live-bus state, and current-location state on one map while the side panel keeps boarding-stop and arrival rows in the same view. In this capture, simulated browser geolocation near Park Avenue and Sagamore Road selects Park Ave + Sagamore St as the boarding stop for a trip to 96 William Street, while Elm St + Russell St remains the exit-side stop. The timing row separates WRTA-derived bus arrival at the boarding stop from the derived destination-arrival estimate.}
...
Stop selection & Interpreted by rider & GPS-based boarding stop, exit stop, or labeled fallback \\
Location context & Not personalized by default & Current location shown when browser GPS is available \\
Timing row & Stop prediction surface & Bus-at-stop time plus derived destination estimate \\
Map behavior & General live operations view & Selected-place framing with route and stop context \\
...
The most important timing correction is semantic. A bus can arrive at a stop, and the rider can arrive at the selected destination later. If the interface collapses those events into one timestamp, the rider has to guess what the time refers to. The current app displays both values as separate claims. `Bus predicted' and `scheduled' are derived from WRTA stop-time output for the selected boarding stop when GPS is available. The destination-arrival value is computed by adding scheduled in-vehicle travel and OSRM exit-stop-to-place walking duration, with live delay propagated when a WRTA prediction is available.
...
The app is better than a general tracker for this personal use case because it removes specific repeated burdens. The rider does not need to count vague map distances, identify a stop from an unlabeled route line, infer whether a location pin is connected to a route, or wait for a static bus marker to jump after the next feed update. The app renders route paths, bus-stop symbols, live buses, place icons, and current location together, and it interpolates live bus markers between source updates so the vehicle appears to move continuously while still depending on WRTA data.
...
For a selected saved destination, the primary path uses browser GPS as the trip origin. The server searches only the tracked route set, evaluates feasible boarding and exit stop pairs, and selects among the least-walking candidates. The UI displays the boarding stop from the current location, the exit stop near the selected place, route context, and upcoming stop arrivals. If GPS is unavailable, the app uses a labeled location-nearest fallback instead of implying that the fallback is the rider's boarding stop.

\subsection{Destination-arrival estimate}

For each selected trip, the server obtains upcoming stop times for the selected boarding stop and requests OSRM walking routes from the current location to the boarding stop and from the exit stop to the selected destination. Destination arrival is calculated as

\[
T_{\mathrm{destination}} = T_{\mathrm{board}} + D_{\mathrm{ride}} + D_{\mathrm{exit\ walk}},
\]

where \(T_{\mathrm{board}}\) is the WRTA-derived boarding-stop time, \(D_{\mathrm{ride}}\) is the scheduled in-vehicle travel time between the boarding and exit stops, and \(D_{\mathrm{exit\ walk}}\) is the OSRM walking duration from the exit stop to the selected place. If OSRM fails, the server falls back to a straight-line walking estimate at the configured walking speed of 1.34 m s\(^{-1}\). The UI labels destination arrival as a source-bounded derived estimate rather than an official WRTA prediction.

\subsection{Visual verification}

The screenshot in Figure~\ref{fig:bus_interface} was generated from the local app at desktop width using simulated browser geolocation near Park Avenue and Sagamore Road with 96 William Street selected. This verifies that the public interaction model has the expected ingredients: current-location marker, selected destination, highlighted route path, bus-stop symbols, live-bus layer, boarding-stop panel, and separate bus-at-stop and destination-arrival fields.
