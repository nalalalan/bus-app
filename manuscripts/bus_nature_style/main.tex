...
The WRTA tracker is route-first: routes and vehicles are the primary objects. The AO Labs interface is location-first: a selected place reframes the map around that destination, the closest tracked stop, route geometry, live buses, and current location when browser GPS is available (Fig.~\ref{fig:bus_interface}). This changes the task from inspecting the transit network to answering a personal question: which stop and which bus time matter for this place?

\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\textwidth]{bus-interface.png}
    \caption{Stable live interface for selected WRTA destinations. The public app keeps saved destinations, route geometry, bus-stop symbols, live-bus state, and current-location state on one map while the side panel keeps closest-stop and arrival rows in the same view. At capture time WRTA returned no stop times for the selected stop; the empty timing row is therefore preserved rather than replaced with a fabricated arrival. This screenshot was generated from the local app using simulated browser geolocation at 96 William Street so that the current-location framing is visible without depending on desktop permission state.}
...
Stop selection & Interpreted by rider & Closest tracked stop shown and labeled \\
...
The most important timing correction is semantic. A bus can arrive at a stop, and the rider can arrive at the selected destination later. If the interface collapses those events into one timestamp, the rider has to guess what the time refers to. The current app displays both values as separate claims. `Bus at stop' is derived from WRTA stop-time output for the nearest tracked stop. `At destination about' is computed by adding the OSRM stop-to-place walking duration to the stop time.
...
\subsection{Closest-stop selection}

For a selected saved destination, the server searches stops from the tracked route set and selects the stop with the smallest straight-line distance to the destination. The UI then displays the closest tracked stop, route context, and upcoming stop arrivals. This is intentionally simple and inspectable; it minimizes walking among the currently tracked routes rather than performing full-system transit planning.

\subsection{Destination-arrival estimate}

For each selected-location arrival row, the server obtains upcoming stop times for the closest tracked stop and requests an OSRM walking route from that stop to the selected destination. Destination arrival is calculated as

\[
T_{\mathrm{destination}} = T_{\mathrm{stop}} + D_{\mathrm{walk}},
\]

where \(T_{\mathrm{stop}}\) is the WRTA-derived stop time and \(D_{\mathrm{walk}}\) is the OSRM walking duration. If OSRM fails, the server falls back to a straight-line walking estimate at the configured walking speed of 1.34 m s\(^{-1}\). The UI labels destination arrival as approximate.

\subsection{Visual verification}

The screenshot in Figure~\ref{fig:bus_interface} was generated from the local app at desktop width using simulated browser geolocation at 96 William Street and a selected Cold Stone destination. This verifies that the public interaction model has the expected ingredients: current-location marker, selected destination, route path, bus-stop symbols, live-bus layer, closest-stop panel, and separate bus-at-stop and destination-arrival fields.
